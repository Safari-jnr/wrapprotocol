...
export async function POST(request: NextRequest) {
  ...
  const supabase = await createServerSupabaseClient();    // Upsert on tx_hash to handle retries/duplicates gracefully
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { error } = await (supabase.from("claims") as any).upsert(
    {
