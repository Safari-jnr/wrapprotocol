...
export function ConnectOrMessage({ onOpenChange }: Props) {
  ...
  return (
    <div ref={ref} className="relative inline-flex flex-col items-center w-full max-w-sm sm:max-w-none">
      ...
      {/* ── Dropdown ── */}
      {open && !showMsg && (
        <div className="mt-3 w-full sm:w-72 rounded-2xl glass border border-white/15 shadow-2xl shadow-black/40 overflow-hidden animate-slide-down">
          ...
          {isConnected ? (
            <>
              ...
              <button
                ...
                className="w-full flex items-center gap-3 px-5 py-4 text-sm font-medium text-white/70 hover:text-white hover:bg-white/8 transition-colors text-left"
              >
                <span className="w-8 h-8 rounded-xl bg-violet-500/20 flex items-center justify-center shrink-0">
            ...
            <>
              {/* Connect Wallet option — opens RainbowKit modal */}
              <button
              ...
              >
                <span className="w-8 h-8 rounded-xl bg-accent-500/20 flex items-center justify-center shrink-0">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/40 mt-0.5">MetaMask, Trust, WalletConnect…</p>
                </div>
              </button>
      ...
      {/* ── Message form ── */}
      {open && showMsg && (
        <div className="mt-3 w-full sm:w-80 rounded-2xl glass border border-white/15 shadow-2xl shadow-black/40 p-5 animate-slide-down space-y-4">
          <div className="flex items-center justify-between">
            <p className="font-bold text-white">Message the Owner</p>
            <button onClick={() => setOpenWithNotify(false)} className="text-white/30 hover:text-white/60 p-1">
          ...
          {msgState === "sent" ? (
            <div className="text-center space-y-2 py-4">
              ...
              <button onClick={() => setOpenWithNotify(false)} className="text-xs text-white/30 hover:text-white/50 underline">Close</button>
            </div>
          ) : (
